...
<script type="text/javaScript" language="javascript">
	...
	******************************************************** */
	...
	function fnEgovSchdulSe(setValue) {
		...
		location.href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageWeekList.do' />?year=<%=iNowYear%>&month=<%=iNowMonth%>&week=<%=iNowWeek%>
	&searchCondition=SCHDUL_SE&searchKeyword="
				+ setValue;
	}
...
<BODY>
	<form name="deptSchdulManageVO" id="deptSchdulManageVO" action=""
		method="post">
					...
					<td height="60">
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							...
							<tr>
								<td width="10px"></td>
								<td width="200px">
								<td><select name="schdulSe" class="select" id="schdulSe"
									...
									<table>
										<tr>
											<td width="80" align="center"><a
												href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageWeekList.do' />?year=<%=iNowYear - 1%>&amp;month=<%=iNowMonth%>&amp;week=<%=iNowWeek%>">
													<img
													src="<c:url value='/images/egovframework/com/cmm/icon/icoPrev.gif' />"
													align="middle" style="margin-right: 4px; border: 0px;"
													alt="이전으로가기아이콘표시">
											</a> <%=iNowYear%>년 <a
												href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageWeekList.do' />?year=<%=iNowYear + 1%>&amp;month=<%=iNowMonth%>&amp;week=<%=iNowWeek%>">
													<img
													src="<c:url value='/images/egovframework/com/cmm/icon/icoNxt.gif' />"
													align="middle" style="margin-left: 4px; border: 0px;"
													alt="다음으로가기아이콘표시">
											</a></td>
											<td width="80" align="center">
												<%
													if (iNowMonth > 0) {
												%> <a
												href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageWeekList.do' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth - 1%>&amp;week=<%=iNowWeek%>">
													<img
													src="<c:url value='/images/egovframework/com/cmm/icon/icoPrev.gif' />"
													align="middle" style="margin-right: 4px; border: 0px;"
													alt="이전으로가기아이콘표시">
 ...
 %>
											...
											<td width="80" align="center">
												<%
													if (iNowWeek > 0) {
												%> <a
												href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageWeekList.do' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth%>&amp;week=<%=iNowWeek - 1%>">
													<img
													src="<c:url value='/images/egovframework/com/cmm/icon/icoPrev.gif' />"
													align="middle" style="margin-right: 4px; border: 0px;"
													alt="이전으로가기아이콘표시">
 ...
 %>
			...
			<table width="100%" border="0" cellpadding="1" cellspacing="1"
				bgcolor="#828486">
				<tr>
					<td width="180px" bgcolor="#E0E0E0" align="center" height="30px">날짜</td>
					<!-- <td width="120px" bgcolor="E0E0E0" align="center">시간</td> -->
					<td width="100px" bgcolor="#E0E0E0" align="center">담당부서</td>
				...
				<tr>
					<td bgcolor="#F5F5F5" align="center" valign="middle" height="50px">
						<%=sTmpDate.substring(0, 4)%>년<%=sTmpDate.substring(4, 6)%>월<%=sTmpDate.substring(6, 8)%>일
						<%=arrDateTitle[i]%>
					</td>
							...
							if (listResult != null) {
									...
									for (int j = 0; j < listResult.size(); j++) {
										...
										if (iUseDate >= iBeginDate && iUseDate <= iBeginEnd) {
											out.print("<table><tr><td nowrap><div class='divDotText' style='width:120px;border:solid 0px;'><a href=\"JavaScript:fn_egov_detail_DeptSchdulManage('"
													+ (String) egovMap.get("schdulId") + "')\">");
							...
							if (listResult != null) {
									...
									for (int j = 0; j < listResult.size(); j++) {
										...
										if (iUseDate >= iBeginDate && iUseDate <= iBeginEnd) {
											...
											out.print((String) egovMap.get("deptName"));
						...
						<%
							if (listResult != null) {
									...
									for (int j = 0; j < listResult.size(); j++) {
										...
										int iBeginEnd = Integer.parseInt(((String) egovMap.get("schdulEndde")).substring(0, 8));
										if (iUseDate >= iBeginDate && iUseDate <= iBeginEnd) {
											out.print("<table><tr><td nowrap><div class='divDotText' style='width:350px;border:solid 0px;'><a href=\"JavaScript:fn_egov_detail_DeptSchdulManage('"
													+ (String) egovMap.get("schdulId") + "')\">");
