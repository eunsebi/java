...
<validator:javascript formName="mtboard" staticJavascript="false" xhtml="true" cdata="false"/>
...
<script type="text/javascript">
	...
	function fn_egov_regist_notice() {
		...
		/* var chckType = document.getElementsByName('location'); 
		...
		if (!validateMtboard(document.mtboard)){
			return;
		}

		if (confirm('<spring:message code="common.regist.msg" />')) {
			//document.board.onsubmit();
			document.mtboard.action = "<c:url value='/com/mtb${prefix}/insertMTBoardArticle.do'/>";
			document.mtboard.submit();
		}
	}

	function fn_egov_select_noticeList() {
		document.mtboard.action = "<c:url value='/com/mtb${prefix}/selectMTBoardList.do'/>";
		document.mtboard.submit();
	}
	...
	******************************************************** */
	function fn_egov_checkAll_number(){
		...
		//undefined
		if( FLength == 1){
			document.mtboard.number.checked = checkAllValue;
		}{
	...
	function fn_egov_checkAll_name_1(){
		...
		//undefined
		if( FLength == 1){
			document.mtboard.name_01.checked = checkAllValue;
		}{
				for(var i=0; i < FLength; i++)
				{
	...
	function fn_egov_checkAll_name_2(){
		...
		//undefined
		if( FLength == 1){
			document.mtboard.name_02.checked = checkAllValue;
		}{
				for(var i=0; i < FLength; i++)
				{
	...
	function fn_egov_checkAll_spec_1(){
		...
		//undefined
		if( FLength == 1){
			document.mtboard.spec_01.checked = checkAllValue;
		}{
				for(var i=0; i < FLength; i++)
				{
	...
	function fn_egov_checkAll_spec_2(){
		...
		//undefined
		if( FLength == 1){
			document.mtboard.spec_02.checked = checkAllValue;
		}{
				for(var i=0; i < FLength; i++)
				{
...
<!-- 메인탑 컨텐츠 시작 -->
	...
	<div id="center_body">
		...
		<div style="float: left; width: 800px;">
			...
			<form:form commandName="mtboard" name="mtboard" method="post" enctype="multipart/form-data" >
					...
					<ul>
						...
						<li class="table_list_left_300">
							<span class="mt_name_left_300">
			    				...
			    				<select id="location" name="location" style="background-color: white;">
					...
					<ul>
						...
						<li class="table_list_left_300">
							<span class="mt_name_left_300">
								...
								<input type="hidden" id = "schdulDeptId" path="schdulDeptId" name="schdulDeptId" />
								<form:errors path="schdulDeptName" />
							</span>
						</li>
						<li class="table_list_left_100">
						...
						<li class="table_list_left_300">
							<span class="mt_name_left_300">
								<input id="changedate" type="hidden" name="changedate" />
				      			<input name="changedateView" type="text" size="10" readOnly onClick="javascript:fn_egov_NormalCalendar(document.board, document.board.changedate, document.board.changedateView);" title="게시시작일자입력">
								<%-- <input name="changedateView" type="text" size="10" value="<%=thisDay %>" readOnly onClick="javascript:fn_egov_NormalCalendar(document.board, document.board.changedate, document.board.changedateView);" title="게시시작일자입력"> --%>
								<a href="#" onClick="javascript:fn_egov_NormalCalendar(document.forms[0], document.forms[0].changedateView);">
								<img src="<c:url value='/images/egovframework/com/cmm/icon/bu_icon_carlendar.gif' />" onClick="javascript:fn_egov_NormalCalendar(document.board, document.board.changedate, document.board.changedateView);" width="15" height="15" alt="달력창팝업버튼이미지"></a>
							</span>
						</li>
					</ul>
					<ul>
						...
						<li class="table_list_left_300">
							<span class="mt_name_left_300">
								...
								<select id="number" name="number" style="background-color: white;">
									<option value="" selected="selected">선택</option>
				            		<option value="1">1호기</option>
				            		<option value="2">2호기</option>
				            		<option value="2">3호기</option>
				            		<option value="2">4호기</option>
				        		</select>
