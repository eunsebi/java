...
<%
 /**
  * @Class Name : MaterialConsumptionList.jsp
...
<title>소모품 재고 현황 관리 조회 화면</title>
...
<!-- 메인탑 컨텐츠 시작 -->
	<div id="center_body">
		<div class="intro_title">
			...
			<div class="intro_title_right"><div class="intro_title_right_h_img"><img src="/images/egovframework/adt/img_home.gif" width="11" height="11" alt="" /></div> HOME &gt; <span class="intro_title_right_text">소모품 재고 현황 관리</span></div>
...
-->
					<%-- <c:if test="${brdMstrVO.authFlag == 'Y'}"> --%>
						<li style="float: right;padding-right:10px">
							<a href="<c:url value='/com/mat/MaterialConsumablesRegister.do'/>?codeId=COM300"><img src="/images/egovframework/adt/btn_regist.gif" border="0" /></a>
						</li>
					<%-- </c:if> --%>
					</ul>
				</div>
									...
									<div style="width:360px; text-overflow:ellipsis; overflow:hidden;">
										<span class="align_left_text">
								            ...
								            <c:choose>
								                ...
								                <c:otherwise>
													<a href="<c:url value='/com/mat/Consumption/selectMaterialConsumablesCode.do'/>?code=<c:out value="${result.code}"/>"  onclick="selectUpdtProgramListDetail('<c:out value="${result.codeNm}"/>'); return false;">
														<c:if test="${fn:length(result.codeNm)> 22}">
															<c:out value="${fn:substring(result.codeNm,0, 22)}"/>...
														</c:if>
														<c:if test="${fn:length(result.codeNm)<= 22}">
