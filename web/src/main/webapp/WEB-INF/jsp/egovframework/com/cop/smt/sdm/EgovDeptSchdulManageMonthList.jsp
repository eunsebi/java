...
<HEAD>
	...
	******************************************************** */
	...
	function fnEgovGroupCode(setValue) {

		location.href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageMonthList.do' />?year=<%=year%>&month=<%=month%>&groupId=" + setValue;
...
<td height="60">
    ...
    <td>
    	그룹설정
     	<select name="groupId" class="select" id="groupId" onChange="fnEgovGroupCode(document.forms[0].groupId.options[document.forms[0].groupId.selectedIndex].value);" title="일정구분선택">
			<c:forEach var="result" items="${groupCode}" varStatus="status">
				<option value='${result.code}' <c:if test="${groupId == result.code}">selected</c:if>>${result.codeNm}</option>
			</c:forEach>
  		</select>
		일정구분
	...
	<td>
		<table>
			...
			<a href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageMonthList.do' />?year=<%=year-1%>&amp;month=<%=month%>"><img src="<c:url value='/images/egovframework/com/cmm/icon/icoPrev.gif' />" align="middle" style="margin-right:4px;border:0px;" alt="이전으로가기아이콘표시"></a>
			<%=year%>년
			<a href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageMonthList.do' />?year=<%=year+1%>&amp;month=<%=month%>"><img src="<c:url value='/images/egovframework/com/cmm/icon/icoNxt.gif' />" align="middle" style="margin-left:4px;border:0px;" alt="다음으로가기아이콘표시"></a>
			</td>
			<td width="80" align="center">
			<%if(month > 0 ){ %>
			<a href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageMonthList.do' />?year=<%=year%>&amp;month=<%=month-1%>"><img src="<c:url value='/images/egovframework/com/cmm/icon/icoPrev.gif' />" align="middle" style="margin-right:4px;border:0px;" alt="이전으로가기아이콘표시"></a>
			<%}%>
			<%=month+1%>월
			<%if(month < 11 ){ %>
			<a href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageMonthList.do' />?year=<%=year%>&amp;month=<%=month+1%>"><img src="<c:url value='/images/egovframework/com/cmm/icon/icoNxt.gif' />" align="middle" style="margin-left:4px;border:0px;" alt="다음으로가기아이콘표시"></a>
			<%}%>
			</td>
			</tr>
			</table>
...
for(int index = 1; index <= endDay; index++)
{
	...
	String sUseDate = Integer.toString(year);		// 해당년도
	...
	*/
	...
	if(listResult != null){
		...
		for(int i=0;i < listResult.size(); i++){
			...
			String aa = (String)egovMap.get("reptitSeCode");
