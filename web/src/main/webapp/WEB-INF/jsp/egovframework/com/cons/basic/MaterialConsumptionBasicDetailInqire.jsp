...
function fn_egov_select_linkPage(pageNo){
	...
	document.frm.action = "/uss/olh/wor/WordDicaryListInqire.do";
   	document.frm.submit();
 ...
 * 수정 처리 함수
 ******************************************************** */
function fn_egov_updt_MaterialConsumptionBasic(){

	alert("공사중!!!");
	document.frm.action = "/com/mat/Consumption/updateMaterialConsumptionCode.do";
	document.frm.submit();	
 ...
 * 설비별 목록회면 처리 함수
 ******************************************************** */
function fn_egov_select_noticeList(pageNo,codeId,mcode){
	document.frm.pageIndex.value = pageNo;
	document.frm.codeId.value = codeId;
	document.frm.mcode.value = mcode;
...
function fn_egov_delete_notice(cmd,number){
	...
	if(confirm("삭제 하시겠습니까?")){
		...
		vFrom.cmd.value = 'del';
		vFrom.action = "<c:url value='/com/mat/Consumption/deleteMaterialConsumptionCode.do' />";
		vFrom.submit();
	}else{
		vFrom.cmd.value = '';
	}
...
<!-- 		<div class="intro_img">
		...
		<div style="float: left; width: 653px;">
			<form:form commandName="searchVO" name="frm" method="post" action="">
				...
				<input type="hidden" name="mcode" value="<c:out value='${result.mcode}'/>" />
				...
				<div class="egovframe_table_detail" >
					<table summary="faq list" cellpadding="" cellspacing="0">
						...
						<tr>
							<th scope="row">제목</th>
							<td colspan="5"><span class="title"><c:out value="${result.codeNm}" /></span></td>
						</tr>
						<tr>
							<th scope="col">작성자</th>
							<td>
						...
						<tr>
							<th scope="col">교체 날짜</th>
							<td>
								<span class="title">
									<input id="dateBgnde" name="dateBgnde" type="hidden"  value='<c:out value="${result.dateBgnde }" />'>
									<input name="dateBeginView" type="text" size="10" value="<c:out value="${result.dateBgnde }" />"  readOnly onClick="javascript:fn_egov_NormalCalendar(document.frm, document.frm.dateBgnde, document.board.dateBeginView);" title="게시시작일자입력">
							...
							<th scope="col">차기 교체 날짜</th>
							<td>
								<span class="title">
									<label for="qestnSj" style="display: none;">차기 교체일</label>
									<input id="dateEndde" name="dateEndde" type="hidden" value='<c:out value="${result.dateEndde }" />'>
									<input name="dateEnddeView" type="text" size="10" value="<c:out value="${result.dateEndde }" />"  readOnly onClick="javascript:fn_egov_NormalCalendar(document.frm, document.frm.dateEndde, document.board.dateEnddeView);" title="게시시작일자입력">
						...
						<tr>
							<th scope="col" style="height: 30px;">교체수량</th>
							<td colspan="5">
								<input id="qty" name="qty" value="<c:out value='${result.qty }' />">
							</td>
						</tr>
						<tr>
							<th scope="col">사용소모품</th>
							<td colspan="5">
								<span class="title">
									<form:select path="code" id="code" title="소모품명">
										<form:option value="${result.code }" label="${result.sdCodeNm }"/>
					...
					<!-- 버튼 시작(상세지정 style로 div에 지정) -->
						...
						<!-- 목록/저장버튼  -->
							<table border="0" cellspacing="0" cellpadding="0" align="center">
								<tr>
									<td>
										<a href="#LINK" onclick="javascript:fn_egov_updt_MaterialConsumptionBasic(); return false;">수정</a>
									</td>
								<c:if test="${result.firstRegisterId == sessionUniqId}">     
									...
									<td width="10"></td>
									<td>
										<a href="#LINK" onclick="javascript:fn_egov_select_noticeList('<c:out value='${searchVO.pageIndex}'/>','<c:out value="${result.codeId}" />','<c:out value="${result.mcode}" />'); return false;">목록</a> 
									</td>
