...
<%
 /**
  * @Class Name : MaterialConsumptionBasicList.jsp
  * @Description : 소모품 Code별 관리 조회 화면
...
<title>소모품 Code별 관리 조회 화면</title>
			...
			<form name="frm" action ="<c:url value='/com/mat/MaterialList.do' />" method="post">
				...
				<h2 class="hidden"><c:out value="${machineNm}"/> 다운로드는 로그인 후 받을 수 있습니다.</h2>
					<ul style="padding-top: 5px;">
						<li class="black_title_text" style="float: left;"><c:out value="${machineNm}"/> 교체 현황</li>
<!--							
...
-->
					<%-- <c:if test="${brdMstrVO.authFlag == 'Y'}"> --%>
						<li style="float: right;padding-right:10px">
							<a href="<c:url value='/com/mat/MaterialCreanRollerRegister.do'/>?codeId=<c:out value='${searchVO.codeId}' />"><img src="/images/egovframework/adt/btn_regist.gif" border="0" /></a>
						</li>
					<%-- </c:if> --%>
					</ul>
				</div>
								...
								<td style="cursor:hand;" nowrap>
									<div style="width:360px; text-overflow:ellipsis; overflow:hidden;">
										<span class="align_left_text">
											<strong>
												<a href="<c:url value='/com/mat/MaterialCreanRollerMachineList.do'/>?codeId=<c:out value="${result.codeId}" />&amp;mcode=<c:out value='${result.mcode}'/>"  onclick="selectUpdtProgramListDetail('<c:out value="${result.codeNm}"/>'); return false;">
