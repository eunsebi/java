...
<!-- 메인탑 컨텐츠 시작 -->
	...
	<!-- ********** 여기서 부터 본문 내용 *************** -->
		<div style="float: left; width: 653px;">
			<form name="frm" action ="<c:url value='/com/mat/MaterialCreanRoller.do' />" method="post">
									...
									<div style="width:360px; text-overflow:ellipsis; overflow:hidden;">
										<span class="align_left_text">
								            ...
								            <c:choose>
								                ...
								                <c:otherwise>
													<a class="board" href="/com/mat/MaterialCreanRollerDetailInqire.do?codeId=<c:out value='${result.codeId }' />&amp;number=<c:out value='${result.number }' />">
														<c:if test="${fn:length(result.codeNm)> 22}">
															<c:out value="${fn:substring(result.codeNm,0, 22)}"/>...
														</c:if>
														<c:if test="${fn:length(result.codeNm)<= 22}">
				...
				<div align="right">
					...
					<ul style="padding-top: 100px;">
						<li style="float: right;padding-right:10px;padding-top: 20px;">
							<a href="/com/mat/MaterialCreanRollerConsumption.do?codeId=<c:out value='${searchVO.codeId}' />"><img src="/images/egovframework/adt/btn_list.gif" border="0" /></a>
						</li>
					</ul>
				</div>
			<!-- Button End -->
