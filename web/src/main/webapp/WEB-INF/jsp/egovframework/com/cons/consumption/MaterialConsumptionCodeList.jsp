...
<%
 /**
  * @Class Name : MaterialConsumptionCodeList.jsp
  * @Description : 소모품 Code별 조회 화면
...
<!-- 메인탑 컨텐츠 시작 -->
	<div id="center_body">
		<div class="intro_title">
			...
			<div class="intro_title_right"><div class="intro_title_right_h_img"><img src="/images/egovframework/adt/img_home.gif" width="11" height="11" alt="" /></div> HOME &gt; <span class="intro_title_right_text">소모품 재고 현황 관리<c:out value="${brdMstrVO.bbsNm}"/></span></div>
	...
	<!-- ********** 여기서 부터 본문 내용 *************** -->
		<div style="float: left; width: 653px;">
			<form name="frm" action ="<c:url value='/com/mat/MaterialList.do' />" method="post">
				...
				<h2 class="hidden"><c:out value="${codeNm}"/> 다운로드는 로그인 후 받을 수 있습니다.</h2>
					<ul style="padding-top: 5px;">
						<li class="black_title_text" style="float: left;"><c:out value="${codeNm}"/></li>
<!--							
									...
									<div style="width:360px; text-overflow:ellipsis; overflow:hidden;">
										<span class="align_left_text">
								            ...
								            <c:choose>
								                ...
								                <c:otherwise>
													<a class="board" href="/com/mat/consumption/MaterialConsumablesDetail.do?codeId=<c:out value='${result.codeId }' />&amp;number=<c:out value='${result.number }' />">
														<c:if test="${fn:length(result.codeNm)> 22}">
															<c:out value="${fn:substring(result.codeNm,0, 22)}"/>...
														</c:if>
														<c:if test="${fn:length(result.codeNm)<= 22}">
				...
				<div align="right">
					...
					<ul style="padding-top: 100px;">
						<li style="float: right;padding-right:10px;padding-top: 20px;">
							<a href="/com/mat/MaterialList.do?codeId=COM300"><img src="/images/egovframework/adt/btn_list.gif" border="0" /></a>
						</li>
					</ul>
				</div>
			<!-- Button End -->
