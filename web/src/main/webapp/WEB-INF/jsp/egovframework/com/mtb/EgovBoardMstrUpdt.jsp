...
<validator:javascript formName="mtboardMaster" staticJavascript="false" xhtml="true" cdata="false"/>
<script type="text/javascript">
	...
	function fn_egov_update_brdMstr(){
		if (!validateMtboardMaster(document.mtboardMaster)){
			return;
		}
		...
		//----------------------------
		var theForm = document.mtboardMaster;
		...
		if (fileAtchPosblAt_value == "Y" && document.mtboardMaster.posblAtchFileNumber.value == 0 ) {
			alert("첨부가능파일 숫자를 1개이상 선택하세요.");
			return;
		}
		
		if (fileAtchPosblAt_value == "N" && document.mtboardMaster.posblAtchFileNumber.value != 0 ) {
			document.mtboardMaster.posblAtchFileNumber.value = 0;
		}
		////--------------------------


		if(confirm('<spring:message code="common.update.msg" />')){
			document.mtboardMaster.action = "<c:url value='/com/mtb/UpdateMTBoardMasterInf.do'/>";
			document.mtboardMaster.submit();
		}
	}

	function fn_egov_select_brdMstrList(){
		document.mtboardMaster.action = "<c:url value='/com/mtb/SelectMTBoardMasterInfs.do'/>";
		document.mtboardMaster.submit();
	}

	function fn_egov_delete_brdMstr(){
		if(confirm('<spring:message code="common.delete.msg" />')){
			document.mtboardMaster.action = "<c:url value='/com/mtb/DeleteMTBoardMasterInf.do'/>";
			document.mtboardMaster.submit();
		}
	}
	...
	function fn_egov_inqire_tmplatInqire(){
		...
		if(retVal != null){
			...
			document.mtboardMaster.tmplatId.value = tmp[0];
			document.mtboardMaster.tmplatNm.value = tmp[1];
		}
	}
</script>
...
<form:form commandName="mtboardMaster" name="mtboardMaster" action="${pageContext.request.contextPath}/com/mtb/SelectMTBoardMasterInfs.do" method="post" >
...
<div id="border" style="width:730px">
	...
	<tr>
      ...
      <td>&nbsp;&nbsp;</td>
      <td><span class="button"><a href="<c:url value='/com/mtb/DeleteMTBoardMasterInf.do'/>" onclick="fn_egov_delete_brdMstr(); return false;"><spring:message code="button.delete" /></a></span></td>
      <td>&nbsp;&nbsp;</td>
