...
function createTree(arrName, vYeobu, checkValue) {
   var startNode, openNode;
	...
	if (treeNodes.length > 0) {
		...
		vHtmlCode +="<div class='egovframe_leftmenu_style'>";
		...
		if (startNode !=0) {
			...
			vHtmlCode +="<div class='leftmenu_title_area'><h2>" + nodeValues[2] + "</h2></div>"
...
function addTreeNode(parentNode, recursedNodes) {
	for (var i = 0; i < treeNodes.length; i++) {
		...
		if (nodeValues[1] == parentNode) {
			...
			vHtmlCodeBg      ="<ul><li class='bottom_border'>";
			vHtmlCodeBgList  ="<ul><li>";
			//vHtmlCodeBg      ="<div class='leftmenu_ul_div'><ul><li class='bottom_border'>";
			//vHtmlCodeBgList  ="<div class='leftmenu_ul_div'><ul><li>";
			...
			if (hasChildNode) {
				vHtmlCode +="<div class='leftmenu_ul_div'>"+vHtmlCodeBg+nodeValues[2]+"</li></ul></div>";
			} else{
				// Start link
				if(recursedNodes.length==1){
				   //vHtmlCode +=vHtmlCodeBg+"<a href=javascript:fn_MovePage('" + i + "');>"+nodeValues[2]+"</a></td></tr>";
				   vHtmlCode +="<div class='leftmenu_ul_div'>"+vHtmlCodeBg+"<a href=javascript:fn_MovePage('" + i + "');>"+nodeValues[2]+"</a></li></div>";
				}else{
				   //vHtmlCode +=vHtmlCodeBgList+"<a href=javascript:fn_MovePage('" + i + "');>"+nodeValues[2]+"</a></td></tr>";
				   vHtmlCode +="<div class='leftmenu_ul_div'>"+vHtmlCodeBgList+"<a href=javascript:fn_MovePage('" + i + "');>"+nodeValues[2]+"</a></li></div>";
				}
			}
