...
public class EgovBBSManageController {
	...
	public String selectBoardArticles(@ModelAttribute("searchVO") BoardVO boardVO, ModelMap model) throws Exception {
		...
		// return "egovframework/com/cop/bbs/cafeBlog_EgovNoticeList";
	...
	@RequestMapping("/cop/bbs/addSelectBoardArticle.do")
	public String addSelectBoardArticle(@ModelAttribute("searchVO") BoardVO boardVO, ModelMap model) throws Exception {
		LoginVO user = (LoginVO) EgovUserDetailsHelper.getAuthenticatedUser();
		Boolean isAuthenticated = EgovUserDetailsHelper.isAuthenticated();
		...
		return "egovframework/com/cop/bbs/EgovSelectNoticeRegist";
	}
	...
	public String insertBoardArticle(final MultipartHttpServletRequest multiRequest, @ModelAttribute("searchVO") BoardVO boardVO, @ModelAttribute("bdMstr") BoardMaster bdMstr,
		...
		if (isAuthenticated) {
			...
			if (board.getSchduleData_1().equals("1")) {
				board.setSchduleData_1(board.getSchduleData_1()); // 스케쥴 데이타 추가 (2015.09.06)
			} else {
				board.setSchduleData_1("0");
			}

			if (board.getSchduleData_2().equals("1")) {
				board.setSchduleData_2(board.getSchduleData_2()); // 스케쥴 데이타 추가 (2015.09.06)
			} else {
				board.setSchduleData_2("0");
			}

			if (board.getSchduleData_3().equals("1")) {
				board.setSchduleData_3(board.getSchduleData_3()); // 스케쥴 데이타 추가 (2015.09.06)
			} else {
				board.setSchduleData_3("0");
			}
			...
			// 스케쥴 등록 (2015.09.10)
			board.setSchdule(board.getSchduleData_1() + "," + board.getSchduleData_2() + "," + board.getSchduleData_3());

			System.out.println("스케쥴 : " + board.getSchdule());

			// 스케쥴 등록 (2015.09. 06)
			schduleAdd(board.getSchdule(), board);
	...
	public void schduleAdd(String schdule, Board board) throws Exception {
		 ...
		 * 
		 * CmmnDetailCode vo =
		 * cmmnDetailCodeManageService.selectCmmnDetailCodeDetail
		 * (cmmnDetailCode);
		 */

		DeptSchdulManageVO deptSchdulManageVO = new DeptSchdulManageVO();

		String[] result = schdule.split(",");

		String strDate = null;
		String strDate_mir = null;
		String schdulDeptId = null;

		if (board.getBbsId().equals("BBSMSTR_000000000012")) {
			schdulDeptId = "ORGNZT_0000000000011"; // T1공장
		} else if (board.getBbsId().equals("BBSMSTR_000000000002")) {
			schdulDeptId = "ORGNZT_0000000000000"; // T2공장
		}

		if (result[0].equals("1")) { // 에칭 스탭 비치

			try {
				date = dateFormat.parse(mTime);
			...
			deptSchdulManageVO.setSchdulDeptId(schdulDeptId);
			...
			egovDeptSchdulManageService.insertDeptSchdulManage(deptSchdulManageVO); // 차기일정 스켸쥴 등록
			
		}
		
		if (result[1].equals("1")) { // 전열로 프로파일 비치
			...
			String schdulNm = "전열로 프로파일 비치 예정일";
			deptSchdulManageVO.setSchdulNm(schdulNm);
			deptSchdulManageVO.setSchdulCn(schdulNm);
		...
		if (result[2].equals("1")) { // 전열로,MIR, 박스오븐 프로파일 비치
			...
			String schdulNm = "전열로, MIR건조로, 박스오븐 프로파일 비치 예정일";
			deptSchdulManageVO.setSchdulNm(schdulNm);
			deptSchdulManageVO.setSchdulCn(schdulNm);
			...
			Calendar cal_mir = Calendar.getInstance();
			cal_mir.setTime(date);
			
			cal_mir.add(Calendar.DATE, 7);
			strDate_mir = dateFormat.format(cal_mir.getTime());

			String schdulNm_mir = "전열로 프로파일 비치 예정일";
			deptSchdulManageVO.setSchdulNm(schdulNm_mir);
			deptSchdulManageVO.setSchdulCn(schdulNm_mir);

			String schdulBgnde_mir = strDate_mir + "0000";
			String schdulEndde_mir = strDate_mir + "0000";

			deptSchdulManageVO.setSchdulSe("1");
			deptSchdulManageVO.setSchdulIpcrCode("A");
			deptSchdulManageVO.setSchdulDeptId(schdulDeptId);
			deptSchdulManageVO.setReptitSeCode("1");
			deptSchdulManageVO.setSchdulBgnde(schdulBgnde_mir);
			deptSchdulManageVO.setSchdulEndde(schdulEndde_mir);
			deptSchdulManageVO.setSchdulChargerId(user.getUniqId());
			deptSchdulManageVO.setSchdulKindCode("1");
			deptSchdulManageVO.setFrstRegisterId(user.getUniqId());
			deptSchdulManageVO.setLastUpdusrId(user.getUniqId());
	...
	public String updateBoardArticle(final MultipartHttpServletRequest multiRequest, @ModelAttribute("searchVO") BoardVO boardVO, @ModelAttribute("bdMstr") BoardMaster bdMstr,
		...
		if (isAuthenticated) {
		   ...
		   if (board.getSchduleData_2().equals("1")) {
			...
			// 디비 스케쥴과 수정 스케쥴 비교
			String[] newSchdule = (board.getSchduleData_1()+","+board.getSchduleData_2()+","+board.getSchduleData_3()).split(",");
			String[] oldSchdule = board.getSchdule().split(",");
			
			// 에칭 스탭
			if(!newSchdule[0].equals(oldSchdule[0])) {
				if(oldSchdule[0].equals("0") && newSchdule[0].equals("1")){
					schduleAdd("1,0,0", board);
				}
			}
			
			// 전열로 프로파일
			if(!newSchdule[1].equals(oldSchdule[1])) {
				if(oldSchdule[1].equals("0") && newSchdule[1].equals("1")){
					schduleAdd("0,1,0", board);
				}
			}
			
			// 전체 프로파일
			if(!newSchdule[2].equals(oldSchdule[2])) {
				if(oldSchdule[2].equals("0") && newSchdule[2].equals("1")){
					schduleAdd("0,0,1", board);
				}
			}
	...
	public String deleteBoardArticle(@ModelAttribute("searchVO") BoardVO boardVO, @ModelAttribute("board") Board board, @ModelAttribute("bdMstr") BoardMaster bdMstr, ModelMap model) throws Exception {
	...
	public String insertAnonymousBoardArticle(final MultipartHttpServletRequest multiRequest, @ModelAttribute("searchVO") BoardVO boardVO, @ModelAttribute("bdMstr") BoardMaster bdMstr,
			@ModelAttribute("board") Board board, BindingResult bindingResult, SessionStatus status, ModelMap model) throws Exception {
	...
	public String deleteAnonymousBoardArticle(@ModelAttribute("searchVO") BoardVO boardVO, @ModelAttribute("board") Board board, @ModelAttribute("bdMstr") BoardMaster bdMstr, ModelMap model)
			throws Exception {
	...
	public String updateAnonymousBoardArticle(final MultipartHttpServletRequest multiRequest, @ModelAttribute("searchVO") BoardVO boardVO, @ModelAttribute("bdMstr") BoardMaster bdMstr,
			@ModelAttribute("board") Board board, BindingResult bindingResult, ModelMap model, SessionStatus status) throws Exception {
	...
	public String replyAnonymousBoardArticle(final MultipartHttpServletRequest multiRequest, @ModelAttribute("searchVO") BoardVO boardVO, @ModelAttribute("bdMstr") BoardMaster bdMstr,
			@ModelAttribute("board") Board board, BindingResult bindingResult, ModelMap model, SessionStatus status) throws Exception {
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/cop/bbs/selectBoardArticlePopup.do")
	public String selectBoardArticlePopup(@ModelAttribute("searchVO") BoardVO boardVO, ModelMap model) throws Exception {
		LoginVO user = (LoginVO) EgovUserDetailsHelper.getAuthenticatedUser();
		...
		return "egovframework/com/cop/bbs/EgovNoticeInqirePopup";
	}
}
