 ...
 */
@Controller
public class ConsumableFilterController extends ConsumableTool{
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/com/cons/consumableFilerUseList.do")
	public String consumableFilerUseList(
			@ModelAttribute("searchVO") ComDefaultVO searchVO,
			@CommandMap Map<String, String> commandMap,
			ConsumableInventoryVO consumableInventoryVO,
			ModelMap model) throws Exception {
		
		Loger.info("consumableFilerUseList " );
		
		model.addAttribute("consumableInventoryVO", consumableInventoryVO);
		
		return "egovframework/com/cons/use/Filter/FilterUseList";
	}
	...
	@RequestMapping("/com/cons/consumableFilterUseRegister.do")
	public String consumableFilterUseRegister(
			//@CommandMap Map<String, String> commandMap,
			@ModelAttribute("searchVO") ConsumableInventoryVO consumableInventoryVO,
			ModelMap model) throws Exception {
		
		System.out.println("=========================== 필터 등록페이지 ============================");
		Loger.info("Filter Controller bbsId : {} " , consumableInventoryVO.getBbsId());
		Loger.info("Filter Controller codeId : {} " , consumableInventoryVO.getCodeId());
		Loger.info("Filter Controller Mode : {} " , consumableInventoryVO.getMode());
		Loger.info("Filter Controller pageIndex : {} " , consumableInventoryVO.getPageIndex());
		...
		List<?> schdulDeptName = egovMeetingManageService.egovMeetingManageLisAuthorGroupPopup(comDefaultVO);
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	@RequestMapping("/com/cons/consumableFilterUseRegisterActor.do")
	public String consumableFilterUseRegisterActor(
		...
		String sLocationUrl = "forward:/com/cons/consumableFilterUseRegister.do";
		...
		if (isAuthenticated) {
			...
			// 소모품 3번
			if (consumableInventoryVO.getUseCn_3().length() != 0) {
				map.put("consumableImsi", consumableInventoryVO.getConsumableImsi_3());
				map.put("useCn", consumableInventoryVO.getUseCn_3());
				map.put("sLocationUrl", sLocationUrl);
			...
			// 소모품 4번
			if (consumableInventoryVO.getUseCn_4().length() != 0) {
				map.put("consumableImsi", consumableInventoryVO.getConsumableImsi_4());
				map.put("useCn", consumableInventoryVO.getUseCn_4());
				map.put("sLocationUrl", sLocationUrl);
			...
			// 소모품 5번
			if (consumableInventoryVO.getUseCn_5().length() != 0) {
				map.put("consumableImsi", consumableInventoryVO.getConsumableImsi_5());
				map.put("useCn", consumableInventoryVO.getUseCn_5());
				map.put("sLocationUrl", sLocationUrl);
			...
			// 스켸쥴 등록
			if (consumableInventoryVO.getPm().equals("Y")) {
				String des = "des";
				String dev = "dev";
				if (consumableInventoryVO.getCodeId().equals("COM130")) {			// D.E.S
					fillterSchduleAdd(des, 7, consumableInventoryVO);		// 필터 1차 교쳬 예정일
					fillterSchduleAdd(des, 14, consumableInventoryVO);		// 필터 2차 교쳬 예정일
					fillterSchduleAdd(des, 21, consumableInventoryVO);		// PM 예정일
				} else if(consumableInventoryVO.getCodeId().equals("COM160")){		// 실버현상기
					fillterSchduleAdd(dev, 7, consumableInventoryVO);		// 필터 1차 교쳬 예정일
					fillterSchduleAdd(dev, 14, consumableInventoryVO);		// 필터 2차 교쳬 예정일
					fillterSchduleAdd(dev, 21, consumableInventoryVO);		// PM 예정일
					fillterSchduleAdd(dev, 15, consumableInventoryVO);		// 슬러지 제거 하는 날
				}
		...
		Loger.info("등록 완료후 sLocationUrl : {} ", sLocationUrl);
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	@RequestMapping("/com/cons/use/consumableFilterUseModify.do")
	public String consumableFilterUseModify(
		...
		String sLocationUrl = "egovframework/com/cons/use/Filter/ConsumableFilterUseModify";
	...
	@RequestMapping("/com/cons/use/consumableFilterUseModifyActor.do")
	public String consumableFilterUseModifyActor(
       ...
       oldUseCn_3 = oldResult.getUseCn_3();
       oldUseCn_4 = oldResult.getUseCn_4();
       oldUseCn_5 = oldResult.getUseCn_5();
       ...
       if (codeId.equals("COM130")) {		// DES
    	   ...
    	   newUseCn_3 = consumableInventoryVO.getUseCn_3();
    	   newUseCn_4 = consumableInventoryVO.getUseCn_4();
    	   newUseCn_5 = consumableInventoryVO.getUseCn_5();
      ...
      /* Loger.info("수정전 호기 is : {} ", oldFacility);
       ...
       Loger.info("수정후 대입 수량_5 is : {} ", newUseCn_5);*/
		...
		if (sCmd.equals("modify")) {
			...
			if (newUseCn_3.length() != 0) {
				map.put("consumableImsi", consumableInventoryVO.getConsumableImsi_3());
				map.put("oldUseCn", oldUseCn_3);
				map.put("newUseCn", newUseCn_3);
				url = calculatorModify(map);	// 소모품 재고 수정
			}
			
			if (newUseCn_4.length() != 0) {
				map.put("consumableImsi", consumableInventoryVO.getConsumableImsi_4());
				map.put("oldUseCn", oldUseCn_4);
				map.put("newUseCn", newUseCn_4);
				url = calculatorModify(map);	// 소모품 재고 수정
			}
			
			if (newUseCn_5.length() != 0) {
				map.put("consumableImsi", consumableInventoryVO.getConsumableImsi_5());
				map.put("oldUseCn", oldUseCn_5);
				map.put("newUseCn", newUseCn_5);
				url = calculatorModify(map);	// 소모품 재고 수정
			}
