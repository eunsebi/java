...
public class EgovMainMenuManageController {
    ...
    public String selectMainMenuIndex(
    	...
    	LoginVO user = 
        	...
        	if (chkURL.equals("listNoteRecptn")) {
        		List list_submenu = menuManageService.selectMainSubHead(menuManageVO);
        		
        		model.addAttribute("list_headmenu", list_submenu);
        	} else {
    ...
    public String selectMainMenuHome(
		...
		Map<String, Object> map = bbsMngService.selectBoardArticles(boardVO, "BBSA03");
		model.addAttribute("notiList", map.get("resultList"));
		
		// 공지사항 메인컨텐츠 조회 끝 -----------------------------------

		// T1 Maint Inform 메인 컨텐츠 조회 시작 -------------------------------
		boardVO.setPageUnit(4);
		boardVO.setPageSize(4);
		
		boardVO.setBbsId("BBSMSTR_000000000012");	//T1 Potho Maint Inform
		...
		model.addAttribute("T1_informList", bbsMngService.selectBoardArticles(boardVO, "BBSA03").get("resultList"));

		// T2 Maint Inform  메인 컨텐츠 조회 시작 -------------------------------
		boardVO.setPageUnit(4);
		boardVO.setPageSize(4);
		boardVO.setBbsId("BBSMSTR_000000000002");	//T2 Potho Maint Inform
		...
		model.addAttribute("T2_informList", bbsMngService.selectBoardArticles(boardVO, "BBSA03").get("resultList"));
		
        // MainPage 쪽지 갯수 조회
        
        int totCnt = (Integer) egovNoteRecptnService.selectNoteRecptnRelationCntOpenYn(user.getUniqId());
        model.addAttribute("NoteRecptnRelationCnt", totCnt);
