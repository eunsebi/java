...
public class EgovNotificationController {
	...
	notificationVO.setUniqId(user.getUniqId());
	...
	if (! notificationService.checkNotification(notification)) {
	    model.addAttribute("msg", egovMessageSource.getMessage("uss.ion.noi.alertNtfcTime"));
	    return "egovframework/com/uss/ion/noi/EgovNotificationRegist";
	}
	...
	if (isAuthenticated) {
	    ...
	    // 2015. 3. 10 이중등록 방지 제대로 작동안하여 임시 삭제
	...
	if (isAuthenticated) {
	    notification.setLastUpdusrId(user.getUniqId());
