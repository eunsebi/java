...
public class EgovWebEditorImageController {
    ...
    @RequestMapping(value="/utl/wed/daumeditorInsertImage.do", method=RequestMethod.GET)
    public String goDaumeditorInsertImage() throws Exception {

	return "egovframework/com/utl/wed/DaumeditorInsertImage";
    }
     ...
     * @throws Exception
     */
    @RequestMapping(value="/utl/wed/daumeditorInsertImage.do", method=RequestMethod.POST)
    public String daumeditorImageUpload(HttpServletRequest request, Model model) throws Exception {
	// Spring multipartResolver 미사용 시 (commons-fileupload 활용)
	//List<EgovFormBasedFileVo> list = EgovFormBasedFileUtil.uploadFiles(request, uploadDir, maxFileSize);
	...
	if (list.size() > 0) {
	    ...
	    model.addAttribute("url", url);
	    model.addAttribute("contentType", vo.getClass());
	    model.addAttribute("filename", vo.getFileName());
	    model.addAttribute("filesize", vo.getSize());
